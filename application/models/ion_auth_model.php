...
class Ion_auth_model extends CI_Model
{
	 ...
	 * @author Mathew
	 **/
	public function register($username, $password, $email, $additional_data = array(), $groups = array(), $brigadies = 0, $brigadies2 = 0)
	{
    ...
    public function validate_login($identity, $password)
	{
		if (empty($identity) || empty($password))
		{
			return FALSE;
	...
	public function clear_login_attempts($identity, $expire_period = 86400) {
		...
		public function update_groups_users($groups = NULL) {
		if ($groups != NULL) {
			$query = $this->db->query("SELECT `users_groups`.`group_id` FROM `users_groups` WHERE `users_groups`.`user_id` = '" . $groups['user_id'] . "'");
			if ($query->num_rows() > 0) {
				$this->db->where('user_id', $groups['user_id']);
				$result = $this->db->update('users_groups', $groups);
				if($result == FALSE) {
					return FALSE;
				} else {
					return TRUE;
				...
				$result = $this->db->insert('users_groups', $groups);
                if($result == FALSE) {
                    return FALSE;
                } else {
                    return TRUE;
...
/*
	...
	*
	*   $branch_id	=	id записей которые нужно удалить;
	*   $table_name	=	название таблицы из которой удаляется запись;
	*	$field_name	=	название поля таблицы.
*/
	public function delete_user($id = NULL, $table_name = NULL) {
		if($id != NULL && $table_name != NULL) {
			$delete = $this->db->query("DELETE FROM `" . $table_name . "` WHERE `" . $table_name . "`.`id` IN (".implode(",",$id).")");
			if($delete == FALSE) {
				return FALSE;
			} else {
				return TRUE;
	...
	*	$field_name	=	название поля таблицы.
*/
	public function delete($id = NULL, $table_name = NULL, $field_name = NULL) {
		if ($field_name === NULL) {
			$field_name = $table_name.'_id';
		}
		if($id != NULL && $table_name != NULL) {
			$delete = $this->db->query("DELETE FROM `" . $table_name . "` WHERE `" . $table_name . "`.`" . $field_name . "` IN (".implode(",",$id).")");
			if($delete == FALSE) {
				return FALSE;
			} else {
				return TRUE;
	...
	/**
	 * Доступ пользователя к записям об объектах в работе.
	 * 
	 * @return array
	 * @author Yakovenko Dmitry
	**/

	public function privileges_user($user_id = NULL, $object = NULL, $branch_id = NULL) {
		if ($user_id != NULL && $object == NULL) {
			$privileges = $this->db->query("SELECT `privileges_user`.`privileges_user_id`, `privileges_user`.`object_id`, `branch_organization`.`branch_id`,	`branch_organization`.`name_organization`, `branch_organization`.`registration_address`, `branch_organization`.`email`, `branch_organization`.`face_head`, `branch_organization`.`postal_address` FROM `privileges_user` INNER JOIN `branch_organization` ON `branch_organization`.`branch_id` = `privileges_user`.`object_id` WHERE `privileges_user`.`user_id` ='" . $user_id . "'");
			if($privileges->num_rows() > 0)
			{
				return $privileges->result_array();
			} else {
				return FALSE;
			}
		} elseif ($user_id != NULL && $object == 1) {
			$object_list = $this->db->query("SELECT `branch_organization`.`branch_id`,	`branch_organization`.`name_organization`, `branch_organization`.`registration_address`, `branch_organization`.`main_branch` FROM `branch_organization` WHERE `branch_organization`.`branch_id` NOT IN (SELECT `privileges_user`.`object_id` FROM `privileges_user` WHERE `privileges_user`.`user_id` = '" . $user_id . "') AND `branch_organization`.`main_branch` = '0'");
			if($object_list->num_rows() > 0)
			{
				return $object_list->result_array();
			} else {
				return FALSE;
			}
		} elseif ($user_id != NULL && $object == 2 && $branch_id != NULL) {
				$sql = "";
				foreach($branch_id as $branch_id_value) {
                    $sql .= "('" .$user_id. "', '" .$branch_id_value. "', 1, 0, 0, 0), ";
                }
                $result = $this->db->query("INSERT INTO `privileges_user` (`user_id`, `object_id`, `privileges_review`, `privileges_create`, `privileges_edit`, `privileges_delete`) VALUES ".rtrim($sql, ', '));
				if($result == FALSE) {
					return FALSE;
				} else {
					return TRUE;
	 ...
	 * @author Yakovenko Dmitry
	**/
	public function privileges_rules($user_id = NULL, $branch_id = NULL, $rules = NULL) {
		if ($user_id != NULL && $branch_id != NULL && $rules == FALSE) {
			$privileges = $this->db->query("SELECT `privileges_user`.`privileges_user_id`, `privileges_user`.`object_id`, `privileges_user`.`privileges_review`, `privileges_user`.`privileges_create`,	`privileges_user`.`privileges_edit`, `privileges_user`.`privileges_delete` FROM `privileges_user` WHERE `privileges_user`.`object_id` = '" . $branch_id . "' AND `privileges_user`.`user_id` = '" . $user_id . "'");
			if($privileges->num_rows() > 0) {
				return $privileges->row_array();
			} else {
				return FALSE;
			}
		} elseif ($user_id == NULL && $rules != FALSE) {
			//$result = $this->db->query("UPDATE `privileges_user` SET `privileges_user`.`privileges_review` = '" . $rules['privileges_review'] . "', `privileges_user`.`privileges_create` = '" . $rules['privileges_create'] . "', `privileges_user`.`privileges_edit`= '" . $rules['privileges_edit'] . "', `privileges_user`.`privileges_delete`= '" . $rules['privileges_delete'] . "' WHERE `privileges_user`.`privileges_user_id` = '" . $rules['privileges_user_id'] . "'");
			$this->db->where('privileges_user_id', $rules['privileges_user_id']);
			$result = $this->db->update('privileges_user', $rules);
			if($result == FALSE) {
				return FALSE;
			} else {
				return TRUE;
